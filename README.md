...
<head>
  ...
  <title>Juego Torre de Control - Tráfico y Móvil (nariz según movimiento)</title>
  <style>
    :root{
      --header-h:64px;
      --sky:#87CEEB;
    ...
    /* 2. MEJORA MÓVIL: Forzar pantalla completa y horizontal si es posible */
    @media (max-width: 900px) and (orientation: portrait) {
      /* Puedes agregar un mensaje o una capa aquí si quieres forzar la rotación */
    }
    ...
    /* SVG de avión orientado hacia la derecha por defecto (0° = derecha) */
    .plane .tag{
    ...
    @media (max-width:700px){
      ...
      /* Ajuste de diseño en móviles si es necesario */
...
<body>
  ...
  <div id="modalTutorial" class="modal" role="dialog" aria-hidden="true">
    <div class="card">
      <div class="close" id="closeTutorial">✕</div>
      <h3>Tutorial</h3>
      <p>Presiona Iniciar. Selecciona un avión haciendo click sobre él. Usa el micrófono (G o botón) y di comandos como: "velocidad 150", "rumbo 90", "altitud 8000".</p>
    </div>
  </div>

  <div id="modalCollision" class="modal" role="dialog" aria-hidden="true">
    <div class="card">
      <div class="close" id="closeCollision">✕</div>
      <h3>Colisión detectada</h3>
  ...
  /* ----------------------------------------------------------
     Versión corregida: 
     - Mantiene tu código original (voz, colisiones, modales).
     - **MEJORA 1**: Control de tráfico por Checkbox (Leve, Medio, Fuerte).
     - **MEJORA 2**: Ajustes CSS para mejor visualización en móvil (horizontal).
     - Corrige la orientación del icono para que la **nariz** del avión 
       apunte exactamente en la dirección del movimiento (vx,vy).
     ---------------------------------------------------------- */

  // ========== CONFIG ==========
  const KNOT_TO_PPS = 0.02; // escala (puedes ajustar si quieres más rápido)
  // BASE_SPAWN_INTERVAL es ahora solo un valor grande. El intervalo real se calcula en base al tráfico deseado por minuto.
  ...
  const SVG_ROTATION_ADJUST = 0; // Ajuste extra si tu SVG está orientado distinto (0,90,-90,180)
  ...
  // 1. Lógica para calcular el intervalo según el tráfico
  function calculateSpawnRate(){
    ...
    switch(selectedLevel){
      ...
      case 'fuerte':
        ...
        break;
      default:
        return 0; // Tráfico nulo si no hay selección
    }
  ...
  // ========== VISUAL UPDATE ==========
  // Actualiza posición, texto y rotación del icono según VECTOR de velocidad
  function updatePlaneVisual(pl){
    if(!pl || !pl.el) return;
    // posicionar el contenedor
    pl.el.style.transform = `translate(${pl.x}px, ${pl.y}px)`;

    // calcular ángulo real del vector de movimiento (vx, vy)
    ...
    // atan2 devuelve ángulo en rad respecto al eje X (derecha = 0)
    let angleDeg = Math.atan2(vy, vx) * 180 / Math.PI;
    if(!isFinite(angleDeg)) angleDeg = pl.heading; // fallback
    // aplicar ajuste si la SVG necesita rotación extra
 ...
 <div class="icon" aria-hidden="true">
  <svg viewBox="0 0 120 80" xmlns="http://www.w3.org/2000/svg">
    ...
    // pequeña comunicación de aproximación
    setTimeout(()=>{
      if(planes.has(plane.id)){
        speak(`${plane.name} solicita aproximación.`);
      }
    }, 7000);
  ...
  function loop(ts){
    ...
    // 1. Lógica de spawn mejorada con aleatoriedad (Opción 1)
    ...
    planes.forEach((pl)=>{
      ...
      // Chequeo de aterrizaje: si está dentro del rectángulo de la pista (aprox)
    ...
    toRemove.forEach(id=>{
      if(planes.has(id)){
        ...
        p.el.remove();
        planes.delete(id);
        // Si el avión removido estaba seleccionado, deseleccionar
        if(selectedPlaneId === id) selectPlane(null); 
      }
    });

    // detección de proximidad/colisión (pairwise)
    // ================== DETECCIÓN MEJORADA CON SONIDO ==================
    const WARNING_SOUND = new Audio(''); // tu archivo de radar
    WARNING_SOUND.volume = 0.3; // volumen bajo
    const WARNING_DIST = 60;      // píxeles para advertencia
    const WARNING_ALT = 2000;     // pies de diferencia para advertencia
    const DANGER_DIST = 12;       // píxeles para choque
    ...
    planes.forEach((a)=>{
      ...
      if(!anyNear){
        ...
        a.el.classList.remove('danger'); // Limpiar danger si estaba cerca pero no chocó
        a.warningSoundPlayed = false;
      }
    });
    // ================== FIN DETECCIÓN MEJORADA ==================
  ...
  function showCollisionModal(list){
    ...
    list.forEach(p=>{
      ...
      d.innerHTML = `<strong>${p.name}</strong> — Vel ${Math.round(p.speed)} nudos · Rum ${Math.round(p.heading)}° · Alt ${formatNumber(p.altitude)} ft`;
      details.appendChild(d);
    });
    document.getElementById('modalCollision').style.display = 'flex';
    // pausa el juego
    running = false;
    speak('Colisión detectada.');
  }
  ...
  function toggleListening(){
    ...
    if(listening){ recognition.stop(); listening=false; document.getElementById('btnMic').textContent='🎤'; }
    else { recognition.start(); listening=true; document.getElementById('btnMic').textContent='🎙️'; }
  }
  ...
  function handleTranscript(txt){
    ...
    // si nombran avión exacto, seleccionar
    ...
    if(low.includes('velocidad') || low.includes('vel')){
      if(!selectedPlaneId || !planes.has(selectedPlaneId)){ speak('Primero selecciona un avión.'); return; }
      if(n === null){ speak('No encontré el número de velocidad.'); return; }
    ...
    if(low.includes('rumbo') || low.includes('rumbo a') || low.includes('rumbo,') ){
      if(!selectedPlaneId || !planes.has(selectedPlaneId)){ speak('Primero selecciona un avión.'); return; }
      if(n === null){ speak('No encontré el número de rumbo.'); return; }
      const pl = planes.get(selectedPlaneId);
    ...
    if(low.includes('altitud') || low.includes('pies')){
      if(!selectedPlaneId || !planes.has(selectedPlaneId)){ speak('Primero selecciona un avión.'); return; }
      if(n === null){ speak('No encontré el número de altitud.'); return; }
      const pl = planes.get(selectedPlaneId);
      pl.altitude = n;
      updatePlaneVisual(pl);
  ...
  // iniciar con el número de aviones para tráfico leve (2 o 3, elegimos 3 para empezar)
  function init(){
  ...
  // Iniciar la simulación y el loop
  init();
  requestAnimationFrame(loop);
